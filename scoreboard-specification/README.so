...
*Redis with zset is ideal for applications that require continuous leaderboard updates and can quickly retrieve data in order or score ranges. Also, it can handle loads up to millions. So the core of the system will be around Redis*
...
* For high availability and scalability, we use Load Balancer + Auto Scaling Group, also need read replicas or Redis cluster to meet these general requirements *

## 3. API specification
### Update User Score *`[POST] /api/score/update`*
*This API allows users to complete an action which results in their score being incremented. The system will update the user's score based on the completion of the action.*
...
**Explanation**
- *Action Completion*: This endpoint reflects that a user has completed an action, but the action's details are not important to the API. What matters is the number of points earned as a result of completing the action (`points_earned`).
- *Score Update*: The API receives the `user_id` and `points_earned` from the client, increments the user's score accordingly, and returns a confirmation message along with the updated score.
...
### Get Top 10 Users by Score *`[GET] /api/score/top`*
*This API allows the client to retrieve the top 10 users with the highest scores from the system.*
...
### Get User's Score *`[GET] /api/score/self`*
*This API allows users to get their own score.*
...
*To notify other clients about score changes after a user updates their score via the API, you can use WebSocket to send notifications.*
